...
### 赤ちゃんの横移動
- キー入力（←→ / A D）押下中は `babyX ± babyMoveSpeed*dt`（押している間だけ移動）。キー操作時は移動目標をクリアする。
- タップ/ドラッグした位置を移動目標に設定し、指を離した後もその目標へ1フレーム最大 `babyMoveSpeed*dt` で追従する（タップ移動）。
- ドラッグ（押下）中は指の論理X座標へ移動目標を更新する。押下していないホバー（マウス）では目標を更新しない。
- 可動域は `[48, 312]` にクランプ。
