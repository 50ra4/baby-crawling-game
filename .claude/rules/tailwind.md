...
- カラー: `text-ink` / `text-ink-soft` / `text-primary` / `text-primary-light` / `text-primary-deep` /
  `text-badge` / `text-cream` / `text-cream-deep` / `text-surface` / `text-surface-warm`、
  および対応する `bg-*` / `border-*`（例: `border-border-pink` / `bg-surface`）
- 角丸: `rounded-control`（16px）/ `rounded-card`（20px）/ `rounded-pill`（999px）
